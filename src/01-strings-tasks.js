...
function getRectangleString(width, height) {
  ...
  middleLine = middleLine.repeat(height - 2);
  bottomLine = `└${'─'.repeat(width - 2)}┘\n`;
  result = topLine + middleLine + bottomLine;
  return result;
}
...
function getCardId(value) {
  ...
  if (!Number.isNaN(Number(value.slice(0, value.length - 1)))) {
    result += Number(value.slice(0, value.length - 1)) - 1;
  }
